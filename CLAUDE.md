...
- Do not restart or duplicate the dev server. Do not point DNS anywhere. Do not add analytics, payment, or third-party scripts. ONE owner-directed exception (8 Sep 2026): the Spotify show embed iframe for From the Ground Up on /adapthletics, gated behind `adapthleticsSpotifyShowId` in lib/site.ts (empty = hidden). Set only a VERIFIED id for the founder's own show; no other embeds, widgets, or scripts are authorized by this exception.
